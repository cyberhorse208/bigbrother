 ...
 * 单个调用跟踪
 ...
 > 必须有一个traceid，用来标示最开始的原始调用，能够在后续的调用关系网中传递。根据traceid来识别单个调用的调用关系网。

  
> 必须有个rpcid，用来标示调用层级和路径。

> traceid的传递。比如：场景1下，无须传递traceid；场景2下，系统A、B需要传递traceid；场景3下，系统A需要传递traceid。必须有一种方式来将traceid从接收的请求中传递到派生的到其他系统的请求中去。
 
 
单个调用的跟踪关键在于正确的传递traceid，即入站的请求与派生的出站请求需要匹配，确认派生关系，才能将traceid从入站请求传递到出站请求中。
...
**状态变为TCP_ESTABLISHED的原始请求，对应着最近出站的TCP_SYN_SENT状态请求，也就是“先入先出”。**
 ...
 * 系统调用关系图
 ...
 > 系统调用关系图比较粗糙，只能获得某个系统与其他系统总体的调用关系，无法获得单个调用所产生的调用关系（同一系统的不同调用可能有不同的调用关系，比如系统A在场景1，2，3中的调用关系）。
 
 
## 实现
 
 基于 [/proc/net/tcp](https://github.com/cyberhorse208/bigbrother/blob/master/proc_net_tcp.md)文件内容来实现。**默认假设：/proc/net/tcp中连接的出现时间、状态变化与协议栈中存在的连接有着严格的对应关系**
